...
const Legals: FC<{ slug: string }> = ({ slug }) => {
  const router = useRouter();
  const legalDatas = useLegal(slug);
  ...
  useEffect(() => {
    if (legalDatas.data != undefined) {
      setLegalPageData(legalDatas.data.legalEntries[0]);
      ...
      <div className={styles.container}>
        <section className="page-header">
          <div className="page-header-container">
            <h2> {legalPageData["title"].toUpperCase()}</h2>
          </div>
        </section>
            ...
            <div className="loan-lists">
              <ul className="service-sidebar-menu"  >
                {legalCategoryData.map((_data: any) => {
                  if (selectedslug == _data.slug) {
                    return (
                      <li key={_data.id}>
                        <a className="active" onClick={(e) => { onLegalCategoryChange(e, _data.slug); }}>{_data.title}
